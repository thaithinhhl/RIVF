"""Validate, merge, and summarize a 2Wiki baseline budget run."""
...
RUN_NAME = "2wiki_baselines_b100_openrouter_n1000"
BUDGET = 100
...
GRAFT_RESULTS = Path(
    "results/graft_v2_2wiki_rq1_openrouter_n1000_merged/results.jsonl"
)
...
def validate(rows: list[dict]) -> dict:
    ...
    checks = {
        ...
        "budget_violations": sum(
            row["context_tokens"] > BUDGET for row in rows
        ),
...
def write_summary(path: Path, rows: list[dict], checks: dict) -> None:
    lines = [
        f"# 2WikiMultiHopQA baselines — Bmax={BUDGET}, N=1,000",
        "",
...
def main() -> None:
    global RUN_NAME, BUDGET, ROOT, SHARDS, EXPECTED
    parser = argparse.ArgumentParser()
    parser.add_argument("--budget", type=int, choices=(75, 100), default=100)
    args = parser.parse_args()
    BUDGET = args.budget
    RUN_NAME = f"2wiki_baselines_b{BUDGET}_openrouter_n1000"
    ROOT = Path("results") / f"{RUN_NAME}_merged"
    SHARDS = tuple(
        Path("results") / f"{RUN_NAME}_shard{index}" / "results.jsonl"
        for index in range(8)
